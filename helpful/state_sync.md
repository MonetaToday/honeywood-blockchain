...
```sh
  ...
  sed -i -e 's/trust_height = 0/trust_height = <BLOCK_HEIGHT>/' $DAEMON_HOME/config/config.toml
  sed -i -e 's/trust_hash = ""/trust_hash = "<BLOCK_HASH>"/' $DAEMON_HOME/config/config.toml
